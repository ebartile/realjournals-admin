...
import { CardHeader, MenuItem, Paper, TextField, Typography } from '@material-ui/core';
...
const RegistrationChart = () => {
  ...
  const fetchData = useCallback(() => {
    ...
    request
      .get(route('admin.statistics.registration-chart'), { params })
  ...
  return (
    <ResponsiveCard>
      <CardHeader
        title="Registrations"
        subheader="Monthly Statistics"
        action={<SelectPeriod period={period} onChange={changePeriod} />}
      />
      ...
      <Spin sx={{ flexGrow: 1, height: '100%' }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart margin={{ top: 50, right: 0, left: 0, bottom: 0 }} data={data}>
            ...
            <defs>
              <linearGradient id="registration" x1="0" y1="0" x2="0" y2="1">
...
const SelectPeriod = ({ period, onChange }) => {
  ...
  return (
    <TextField size="small" onChange={onChange} value={period} label="Month" select>
