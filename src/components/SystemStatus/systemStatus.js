  ...
  return (
    <ResponsiveCard sx={{ bgcolor: theme.palette[color].lighter }}>
      <Spin sx={{ flexGrow: 1, height: '100%' }} spinning={loading}>
        <StyledBox>
          <StyledCardContent sx={{ color: theme.palette[color].darker }}>
            <Typography gutterBottom variant="h4">
              {status ? 'Your attention is needed!' : 'All looks good!'}
            </Typography>

            <Typography sx={{ pb: 1 }} variant="body2">
              {status
                ? `There are a total of ${data.total} system logs that need your immediate attention.`
                : 'You will be notified here of any system logs when available.'}
            </Typography>
            ...
            <Box sx={{ pt: 1 }}>
              <Button
                ...
                to={router.generatePath('admin-portal.settings')}
